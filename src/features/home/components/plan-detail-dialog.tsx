  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        className="flex max-h-[min(932px,calc(100svh-4rem))] w-full max-w-[calc(100%-2rem)] flex-col gap-4 overflow-hidden rounded-[20px] bg-white px-6 py-4 text-neutral-900 shadow-figma-lg ring-0 sm:max-w-159 dark:bg-white dark:text-neutral-900"
      >
        {plan && detail && (
          <>
            <div className="flex shrink-0 items-center justify-between">
              <DialogTitle className="type-h3 font-semibold tracking-[-0.144px] text-neutral-800">
                Chi tiết gói
              </DialogTitle>
              <DialogClose
                aria-label="Đóng"
            ...
            <div className="flex min-h-0 flex-1 flex-col gap-5 overflow-y-auto pb-2">
              <div className="flex shrink-0 flex-col gap-5 rounded-2xl bg-neutral-50 p-3">
                ...
                <DialogDescription className="type-p-ui text-neutral-500">
                  {detail.description}
                </DialogDescription>

                <button
                  type="button"
                  className="w-full rounded-full border border-neutral-300 bg-white px-4 py-2 type-body font-medium text-neutral-800 transition-colors duration-200 hover:border-brand hover:text-brand"
                >
                  Xem trên bản đồ
                </button>

                <p className="type-p-ui text-neutral-400">{PLAN_DISCOUNT}</p>
              </div>

              <hr className="shrink-0 border-neutral-300" />

              {detail.sections.map((section) => (
                <section key={section.title} className="flex shrink-0 flex-col gap-2">
                  <h3 className="type-large font-semibold text-neutral-800">{section.title}</h3>
                  <ul className="list-disc ps-6 type-p-ui text-neutral-500">
